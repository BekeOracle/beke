...
contract FundingConstant is Common{
	using SafeMath for uint256;

	event Crowdfunding(address indexed user, uint256 amount, uint256 timestamp);
	event Claim(address indexed user, uint256 BEKEAmount, uint256 USDTAmount, uint256 timestamp);
	...
	uint256 constant FINISHED = 3;
	uint256 constant DOUBLE = 2;

	// BEKE contract address
	IBEKE _BEKEContract;
	address _BEKE;
	...
	// ERC20 of BEKE contract address
...
//	address constant PANCAKE_FACTORY_ADDRESS = 0xBCfCcbde45cE874adCB698cC183deBcF17952812;
	address constant PANCAKE_ROUTER_ADDRESS = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
	address constant PANCAKE_FACTORY_ADDRESS = 0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73;
	...
	function _setInitialCirculation(uint256 userInitial_, uint256 whitelistInitial_) internal{
		_userInitial = userInitial_.mul(10 ** uint256(__BEKEContractERC20().decimals()));
		_whitelistInitial = whitelistInitial_.mul(10 ** uint256(__BEKEContractERC20().decimals()));
	...
	function _setUserInfos(address _address, uint256 _paidAmount) internal{
		...
		if(userInfosLength() == 0){
			//params: userAddress, paidAmount,winning,remain,BEKEOfUser
			_userInfos.push(userInfoStruct(address(0), 0, 0, 0, 0));

			//set  last index to this address
			_indexMapping[address(0)] = userInfosLength().sub(1);
			// Index starts at 1
			_userInfos.push(userInfoStruct(_address, _paidAmount, 0, 0, 0));
			_indexMapping[_address] = userInfosLength().sub(1);
		} else
		// First time to participate in the crowdfunding users
			if(indexMapping(_address) == 0){
				...
				if(__whiteList().isExist(_address)){
					uint256 bekeAmount =  _paidAmount > __maximum() ? __maximum() : _paidAmount;
					uint256 remain = _paidAmount > __maximum() ? _paidAmount.sub(__maximum()) : 0;
					userInfoStruct memory user = userInfoStruct(_address, _paidAmount, 0, remain, bekeAmount);
				...
				//whitelist users
				if(__whiteList().isExist(_address)){
					uint256 bekeAmount =  _paidAmount.add(user.paidAmount) > __maximum() ? __maximum() : _paidAmount.add(user.paidAmount);
	...
	function userInfos() external view returns(uint256 _paidAmount, uint256 _winning, uint256 _remain,uint256 _BEKEOfUser,uint256 _expect){
		userInfoStruct memory user = __userInfos();
		return (user.paidAmount, user.winning, user.remain, user.BEKEOfUser,__canCrowdfunding() == GOING ? getExpect(user) : 0);
	}
	...
	function _setMaximum(uint256 __maximum) internal{
		_maximum = __maximum.mul(10 ** uint256(__BEKEContractERC20().decimals()));
	}
...
// 	}
	...
	function _setEndTime(uint __period) internal{
		_endTime = _startTime.add(__period);
	}
	...
	// @dev function of _BEKEContract
	//		set
	function _setBEKEContract(address contractAddr) internal{
		_BEKEContract = IBEKE(contractAddr);
		_BEKE = contractAddr;
	}

	function initialize(address BEKEcontractAddr, address LPContractAddr, uint256 userInitial_,uint256 whitelistInitial_,  uint256 maximum_, address whiteListContract) external onlyAdmin(){
		_setBEKEContractERC20(BEKEcontractAddr);
		_setBEKEContract(BEKEcontractAddr);
	...
	//		get
	function __BEKEContract() internal view returns(IBEKE){
		return _BEKEContract;
	}
...
// 	}
	...
	// @dev function of _BEKEContractERC20
	//		set
	function _setBEKEContractERC20(address contractAddr) internal{
		_BEKEContractERC20 = IERC20(contractAddr);
	}

	//		get
	function __BEKEContractERC20() internal view returns(IERC20){
		return _BEKEContractERC20;
	}
...
contract Funding is FundingConstant{
	/*
    ...
    function createSwap() external onlyAdmin{
        require(__canCrowdfunding() == COMPLETED,"Not yet");
        _setCanCrowdfunding(FINISHED,0);
		/**
		  * If the actual amount of crowdfunding is more than 300,000,
		  * the total BEKE coin counts will be calculated as 300,000;
        ...
        // The BEKE amount of crowdfunding distribution
        uint256 amount = __coinCount().div(HALF);

        // Transfer BEKE from BEKE __USDTContract
        __BEKEContract().issueTransfer(address(this),__coinCount());
        ...
        TransferHelper.safeApprove(_BEKE, PANCAKE_ROUTER_ADDRESS, amount);
        _setPair(__pancakeFactory().createPair(_USDT,_BEKE));
        _pancakeRouter.addLiquidity(_USDT, _BEKE, usdtEffective, amount, min, min, __owner(), block.timestamp);

        //set pair address  of  lp contract
        _LpContract.setPairAddress(__pair());

		//set pair address  of  BEKE contract
		__BEKEContract().setPairAddress(__pair());
    }
	...
	function claim() external returns(bool result){
		require(__canCrowdfunding() == COMPLETED,"Not yet");
		__BEKEContractERC20().transfer(_msgSender(), __userInfos().BEKEOfUser);
		...
		_userInfos[_indexMapping[_msgSender()]].BEKEOfUser = 0;
		emit Claim(_msgSender(), __userInfos().BEKEOfUser, __userInfos().remain, block.timestamp);
		return true;
	}
     ...
     * and distribute the BEKE quantity
     */
	function iteration(uint256 amount) private{
		for(uint256 i = 1; i < userInfosLength(); i++) {
			...
			uint256 toGeneral = amount.sub(__whitelistEffective());
			...
			user.BEKEOfUser = user.winning.mul(toGeneral).div(totalEffective);
			_userInfos[i] = user;
		}
	}
}