...
contract DAOConstant is Common{
    ...
    struct DAOInfo{
        ...
        // The total number of locked BEKE per pool
        uint256 totalLocked;
    ...
    function initialize(uint256 c_rate_, address BEKEContractAddress_, address crowdfundingContractAddress_) external onlyAdmin() {
        _setC_rate(c_rate_);
        _setBEKEContract(BEKEContractAddress_);
    ...
    function _createDAOPool(string memory _poolName, uint256 _period) internal returns(bool){
        ...
        //last param: set address(0) at first user
        _updateDAOUser(_DAOUsers.length.sub(1), address(0), 0, 0, 0, 0);
        return true;
    }
    ...
    function _updateDAOUser(uint256 _index, address _addr, uint256 _effective, uint256 _reward, uint256 _locked, uint256 _lockedStartTime) internal {
        ...
        _DAOUsers[_index].userCount = _DAOUsers[_index].userCount.add(1);
    }

    function _DAOUserLength() private view returns(uint256){
        return _DAOUsers.length.sub(1);
    }
    ...
    function _canWithdraw(uint256 _index) internal view returns(bool){
        ...
        return lockedStartTime != 0 && lockedStartTime.sub(poolLocked) <= block.timestamp;
    }
    ...
    //		set
    function _setBEKEContract(address contractAddr) internal returns(bool){
        _BEKEContract = IBEKE(contractAddr);
        _setBEKEContractERC20(contractAddr);
        return true;
    }
    ...
    //		set
    function _setBEKEContractERC20(address contractAddr) internal returns(bool){
    ...
    */
    ...
    function checkExpectation(uint256 price_) internal returns(bool){
...
contract DAO is DAOConstant {
    ...
    function deposit(uint256 _poolIndex, uint256 amount) external returns(bool){
        ...
        // Transfer BEKE from user address to this contract address
        __BEKEContractERC20().transferFrom(msg.sender,address(this),amount);
    ...
    function withdraw(uint256 _poolIndex) external returns(bool){
        ...
        __BEKEContractERC20().transfer(_msgSender(),_withdraw(_poolIndex));
        return true;
    }

    // @dev function to get reward
    function reward(uint256 _poolIndex) external returns(bool){
        return __BEKEContractERC20().transfer(_msgSender(),_getReward(_poolIndex));
    }
    ...
    function priceOracle(uint256 nowPrice) external onlyAdmin() returns(bool){
        ...
        uint256 toDAO;
        uint256 toDAOAward;
        (toDAO, toDAOAward)= __BEKEContract().updateUserBalance(flag, _R, DEV_RATE.mul(10 ** DECIMALS1), DAO_RATE.mul(10 ** DECIMALS1), LP_RATE.mul(10 ** DECIMALS1));
        uint256 mint = 0;
        if(!flag){
            if(checkExpectation(nowPrice)){
                // Distribute the reward
                mint = __ICrowdfunding().coinCount();
                __BEKEContract().issueTransfer(address(this),mint);
            }
