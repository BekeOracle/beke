...
contract BEKERate is ERC20UpgradeSafe{
    // total increment
    ...
    modifier onlyAccessAddress() {
        require(_msgSender() == daoAddress() || _msgSender() == crowdfundingAddress(), "require access Address");
        _;
    }

    // @dev function of _\pairAddress
    function setPairAddress(address __pairAddress) external onlyAccessAddress{
        _pairAddress = __pairAddress;
    }


    // Modify the user balance according to the proportion of increase or decrease
    function updateUserBalance(bool _flag, uint256 _rate ,uint256 _developerRate,uint256 _daoRate,uint256 _lpRate) external virtual onlyAccessAddress returns(uint256, uint256){
    ...
    function priceDown(uint256 _rate) internal{
        ...
        for(uint i = 1; i < userInfos.length;i++){
            ...
            if(queryDAOAddressIndex() == i || queryLPAddressIndex() == i || queryPairAddressIndex() == i){
                continue;
            }
    ...
    function priceUp(uint256 _rate ,uint256 _developerRate,uint256 _daoRate,uint256 _lpRate) internal  {
        ...
        for(uint i = 1; i < userInfos.length; i++){
            ...
            if(queryLPAddressIndex() == i || queryPairAddressIndex() == i){
                continue;
            }
            //record DAO award
    ...
    // @dev issuse
    function issueTransfer(address recipient, uint256 amount) external virtual onlyAccessAddress returns (bool) {
    ...
    // query index of Pair address
    function queryPairAddressIndex() internal view returns(uint256){
        return _userIndexMapping[_pairAddress];
    }
